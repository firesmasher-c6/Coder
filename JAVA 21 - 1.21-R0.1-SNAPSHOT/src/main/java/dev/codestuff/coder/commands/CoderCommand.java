...
import dev.codestuff.coder.manager.UserExecutionControl;
...
public class CoderCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        // Handle /coder api <sub> commands
        if (action.equals("api")) {
            if (!configManager.isApiCommandEnabled()) {
                sender.sendMessage("§c[Coder] The api command is disabled in config.yml");
                return true;
            }
            if (args.length < 2) {
                sender.sendMessage("§f[§bCoder§f] §6API subcommands: §fversion");
                return true;
            }
            String apiSub = args[1].toLowerCase();
            if (apiSub.equals("version")) {
                versionManager.handleApiVersionCommand(sender);
                return true;
            }
            sender.sendMessage("§f[§bCoder§f] §cUnknown api subcommand: §f" + args[1]);
            return true;
        }
    ...
    private void showMainHelp(CommandSender sender) {
        ...
        if (configManager.isApiCommandEnabled()) {
            sender.sendMessage("§a/coder api version      §7- Show API version info");
        }
    ...
    public List<String> onTabComplete(CommandSender sender, Command cmd, String alias, String[] args) {
        ...
        if (args.length == 1) {
            ...
            if (configManager.isApiCommandEnabled()) completions.add("api");
