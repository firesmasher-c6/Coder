...
public class BackupManager {
    ...
    private void createZipBackup(File backupFile, CommandSender executor) throws IOException {
        ...
        Path coderPath   = coderFolder.toPath();
        // Resolve the canonical backup path so we can exclude the entire backups/ dir.
        // Without this, previous backup ZIPs get zipped into the new one, inflating
        // the output to multiple GiB on repeated backups.
        Path backupCanonical;
        try {
            backupCanonical = backupFolder.toPath().toRealPath(java.nio.file.LinkOption.NOFOLLOW_LINKS);
        } catch (IOException e) {
            backupCanonical = backupFolder.toPath().toAbsolutePath().normalize();
        }
        final Path excludeDir = backupCanonical;

        try (ZipOutputStream zos = new ZipOutputStream(new BufferedOutputStream(new FileOutputStream(backupFile), 65536))) {
            Files.walkFileTree(coderPath, new SimpleFileVisitor<Path>() {
                ...
                public FileVisitResult preVisitDirectory(Path dir, BasicFileAttributes attrs) {
                    ...
                    Path canonical;
                    try {
                        canonical = dir.toRealPath(java.nio.file.LinkOption.NOFOLLOW_LINKS);
                    } catch (IOException e) {
                        canonical = dir.toAbsolutePath().normalize();
                    }
                    if (canonical.equals(excludeDir)) {
                ...
                public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
                    // Extra guard: skip the output zip itself if it appears mid-walk
                    if (file.toAbsolutePath().normalize().equals(
                            backupFile.toPath().toAbsolutePath().normalize())) {
                        return FileVisitResult.CONTINUE;
                    }
                    Path relative = coderPath.relativize(file);
                ...
                public FileVisitResult visitFileFailed(Path file, IOException exc) {
                    plugin.getLogger().warning("[Backup] Skipping unreadable file: " + file + " — " + exc.getMessage());
                    return FileVisitResult.CONTINUE;
                }
            });
        }
